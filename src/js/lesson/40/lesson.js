...
export default class Lesson extends GeneralLesson {
    ...
    #addTweaks() {
        this.guiControl
            .addColor(
                {clearColor: this.rendererParams.clearColor},
                'clearColor',
            )
            .onChange((value) => {
    ...
    #initModels() {
        ...
        gltfLoader.load(models, (gltf) => {
            const geometries = gltf.scene.children.map(
        ...
        for (const geometry of geometries) {
            ...
            for (let i = 0; i < maxVertices; i++) {
                ...
                if (i < position.count) {
                    ...
                    const index =
                        Math.floor((position.count - 1) * Math.random()) * 3
            ...
            this.meshPositions.push(
                new THREE.BufferAttribute(normalizedPosition, 3),
            )
        }
